...
public class JobPostActivityService {
    ...
    @Transactional
    public boolean deleteRecruiterJob(int jobId, int recruiterId) {
        // Use native deletes to avoid JPA cascading from JobSeekerApply/Save (they are mapped with cascade=ALL).
        jobSeekerApplyRepository.deleteByJobId(jobId);
        jobSeekerSaveRepository.deleteByJobId(jobId);
        return jobPostActivityRepository.deleteRecruiterJob(jobId, recruiterId) > 0;
    }
}
